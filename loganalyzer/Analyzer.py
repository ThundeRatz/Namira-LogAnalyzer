...
class Region:
    ...
    def in_region(self, x, y):
        ...
        if (x > self.top_left[0] and x < self.bottom_right[0]
                and y > self.top_left[1] and y < self.bottom_right[1]):
            return True
        else:
            return False
...
class Analyzer:
    ...
    def update_parameters(self):
        ...
        for agent in self.game.right_team.agents:
            ...
            self.used_stamina_agents_r.append(
                (round(agent.used_stamina, 2), agent.number))

            self.team_moved_distance_r.append(
                (round(agent.moved_distance, 2), agent.number))
            if(agent.moved_distance != 0):
                self.used_per_distance_r.append(
                    (round(agent.used_stamina / agent.moved_distance, 1), agent.number))

        for agent in self.game.left_team.agents:

            agent.used_stamina = self.game.server_param['stamina_max'] + self.game.server_param['stamina_capacity'] - \
                agent.data[self.game.get_play_on_cycles()[-1]]['stamina_capacity'] - agent.data[self.game.get_play_on_cycles()[-1]]['stamina']
            self.used_stamina_agents_l.append(
                (round(agent.used_stamina, 2), agent.number))

            self.team_moved_distance_l.append(
                (round(agent.moved_distance, 2), agent.number))
            if(agent.moved_distance != 0):
                self.used_per_distance_l.append(
                    (round(agent.used_stamina / agent.moved_distance, 1), agent.number))

        if(self.pass_r + self.intercept_l != 0):
            self.pass_accuracy_r = round(
                self.pass_r * 100 / (self.pass_r + self.intercept_l), 1)
        else:
            self.pass_accuracy_r = 0

        if(self.pass_l + self.intercept_r != 0):
            self.pass_accuracy_l = round(
                self.pass_l * 100 / (self.pass_l + self.intercept_r), 1)
        else:
            self.pass_accuracy_l = 0

        if(self.on_target_shoot_l + self.off_target_shoot_l != 0):
            self.shoot_accuracy_l = round(
                self.on_target_shoot_l * 100 / (self.on_target_shoot_l + self.off_target_shoot_l), 1)
        else:
            self.shoot_accuracy_l = 0

        if(self.on_target_shoot_r + self.off_target_shoot_r != 0):
            self.shoot_accuracy_r = round(
                self.on_target_shoot_r * 100 / (self.on_target_shoot_r + self.off_target_shoot_r), 1)
        else:
            self.shoot_accuracy_r = 0
        ...
        for region in self.regions:
            region.ball_in_region_cycles = round(
                region.ball_in_region_cycles * 100 / s, 1)
        ...
        if(self.average_distance_10p_r != 0):
            self.av_st_per_dist_10p_r = round(
                self.average_stamina_10p_r / self.average_distance_10p_r, 1)

        if(self.average_distance_10p_l != 0):
            self.av_st_per_dist_10p_l = round(
                self.average_stamina_10p_l / self.average_distance_10p_l, 1)
        ...
        if key in self.game.ball_pos:
            if(key not in self.play_on_cycles):
                ...
                if(len(kickers) > 0 and kickers[0].team.name == self.game.right_team.name and math.hypot(kickers[0].data[key]['x'] + 51.6, kickers[0].data[key]['y']) < 20 and self.game.ball_pos[key]['Vx']):
                    ball1 = (
                        self.game.ball_pos[key - 1]['x'], self.game.ball_pos[key - 1]['y'])
                    ball2 = (
                        self.game.ball_pos[key]['x'],
                        self.game.ball_pos[key]['y'])
                    if ball1[0] - ball2[0] > 0:
                        (x, y) = self.line_intersection(
                            (ball1, ball2), ((-52.6, 1), (-52.6, 0)))
